...
use crate::utils::merge_sorted_vecs;
...
impl Wire {
    pub fn to_segments(&self, grid_size: f32) -> Vec<WireSegment> {
        let (y0, y1) = (self.start.y, self.end.y); 
        let (x0, x1) = (self.start.x, self.end.x);

        
        // Vertical crossings
        let horz_ts = calc_crossings_t(x0, x1, grid_size);
        
        // Horizontal crossings
        let vert_ts = calc_crossings_t(y0, y1, grid_size);
        ...
        let mut segments = Vec::<WireSegment>::with_capacity(ts_vec.len() - 1);
...
fn calc_crossings_t(x0: f32, x1: f32, grid_size: f32) -> Vec<f32> {
    ...
    let range = if x1 > x0 {
        (x0 / grid_size).ceil() as u32..=(x1 / grid_size).floor() as u32
    } else {
        (x1 / grid_size).ceil() as u32..=(x0 / grid_size).floor() as u32
    };
    ...
    for n in range {
        let t = (n as f32 * grid_size - x0) / (x1 - x0);
        ts.push(t);
    }
